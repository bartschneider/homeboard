...
def get_system_info(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not HAS_PSUTIL:
        info["error"] = "psutil library not available - install with: pip3 install psutil"
        return info
...
def generate_html(system_info: Dict[str, Any]) -> str:
    ...
    html_parts.append(f'<div style="margin-bottom: 10px;"><strong>{hostname}</strong> • Uptime: {uptime}h</div>')
    
    # Create metrics grid
    html_parts.append('<div style="display: flex; justify-content: space-between; text-align: center;">')
    ...
    if "memory" in system_info:
        ...
        html_parts.append(f"""
        <div style="flex: 1; border-left: 1px solid #ccc; border-right: 1px solid #ccc;">
            <div style="font-weight: bold;">Memory</div>
            <div style="font-size: 1.1em;">{memory['percent']:.1f}%</div>
            <div style="font-size: 0.8em;">{memory['used_gb']}/{memory['total_gb']} GB</div>
        </div>
        """)
